...
pre_registered_client_ids = _parse_csv_env("AUTH_PRE_REGISTERED_CLIENT_IDS", default=[])
...
async def _register_pre_registered_clients() -> None:
    """Pre-register known MCP OAuth clients so stateless deployments remain stable."""
    if not auth0_enabled or auth is None or not pre_registered_client_ids:
        return

    for client_id in pre_registered_client_ids:
        try:
            ...
            logger.info("Pre-registered OAuth client_id: %s", client_id)
        except Exception:  # pragma: no cover - defensive runtime path
            logger.exception("Failed pre-registering OAuth client_id: %s", client_id)


async def compose_servers():
    """Import all sub-servers into the main server."""
    await _register_pre_registered_clients()
...
async def server_info(request: Request) -> JSONResponse:
    ...
    return JSONResponse({
        ...
        "authentication": {
            ...
            "pre_registered_client_ids": pre_registered_client_ids if auth0_enabled else []
        },
        "endpoints": {
            "health": "/health",
            "info": "/info",
